...
export default function TextEditor({ onUpdate }) {
  const editor = useEditor({
    extensions: [CharacterCount.configure({ limit: 3000 }), EditorLink, Image, StarterKit, Italic, Bold,HorizontalRule,Underline, TextAlign.configure({
  ...
  return (
    <div className="form-input p-0">
      ...
      <div className="border-[#c9c9c9] border-b-2 flex gap-4 px-4 p-1">
        <button type="button" className="border-r pr-3 pl-1 " onClick={() => editor.chain().focus().undo().run()} title="Undo">
          <UndoIcon />
        </button>
        <button
         type="button"
      ...
      >
        ...
        className={editor.isActive({ textAlign: 'center' }) ? 'is-active border-r pr-3' : 'border-r pr-3'}
          title="Align center">
          <Center/>
        </button>
        <button
        ...
        className={editor.isActive({ textAlign: 'justify' }) ? 'is-active border-r pr-3' : 'border-r pr-3'}
          title="Justify">
          <Right/>
        </button>
        <button
        ...
        <button 
         type="button"
        className="border-r pr-3"
        onClick={() => editor.chain().focus().toggleLink().run()} title="Apply link">
          <LinkIcon />
        </button>
        <button 
         type="button"
        ...
        <button 
         type="button"
        className=""
        onClick={() => editor.chain().focus().toggleCodeBlock().run()} title="Full screen">
          <Fullscreen />
        </button>
