...
const PeopleSearchPerson: React.FC<PeopleSearchPersonProps> = ({
  ...
  return (
    <div
      ...
      className="border-t border-[#C9C9C9] py-[28px]">
        ...
        <div className="flex justify-between items-center">
          ...
          {!isFollowing && (
            <button
              ...
              className="dark:text-white h-[52px] px-[30px] text-[24px] border border-[#C9C9C9] rounded-[10px]">
              Follow
            </button>
          )}
          {isFollowing && (
            <button
              onClick={() => setIsFollowing(!isFollowing)}
              className="relative left-[24px] h-[52px] bg-[#03A9F4] text-white px-[30px] text-[24px] border-[#C9C9C9] rounded-[10px]">
              Unfollow
            </button>
          )}
        </div>
